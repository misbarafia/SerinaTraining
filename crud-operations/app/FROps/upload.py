...
def no_of_files(min_no, max_no,connection_str,containername, dir_path):
    ...
    try:
        ...
        if min_no <= local_upld_cnt <= max_no:
            ...
            no_of_files_msg = "Minimum " + \
                str(min_no) + " files are required for training!"
        elif local_upld_cnt > max_no:
            no_of_files_status = 0
            no_of_files_msg = "Sorry, you can not upload files more than " + \
                str(max_no) + "."
        else:
            no_of_files_status = 0
            no_of_files_msg = "Please ensure, the uploaded directory has a minimum of " + \
                str(min_no)+" to maximum of "+str(max_no) + " files."
...
def upload_blobs(cnt_str, cnt_nm, local_path, old_fld_name):
    ...
    try:
        ...
        for file in blob_list:
            ...
            blob = BlobClient.from_connection_string(conn_str=cnt_str, container_name=cnt_nm, blob_name=file.name)
...
def upload_confirm(cnt_str, cnt_nm, blb_fldr):
    try:
        ...
        upload_confirm_msg = "Files are not uploaded,Please try again after sometime. (" + str(
            e) + ")"
...
def reupload_files_to_azure(file_size_accepted,accepted_file_type,local_path,cnt_str,cnt_nm,old_folder_name,upload_type):
    try:
        check_filetype_status, check_filetype_msg = check_filetype(local_path,cnt_str,cnt_nm, accepted_file_type)
        if check_filetype_status == 1:
            ...
            if fl_status == 1:
                ...
                upload_blobs_status, upload_blobs_msg, upld_cnt, blob_fld_name = upload_blobs(cnt_str, cnt_nm, local_path, old_folder_name)
                upload_confirm_status, upload_confirm_msg = upload_confirm(cnt_str, cnt_nm, old_folder_name)
                fnl_upload_status = 1
        ...
        del_blobs_status, del_blobs_msg, del_cnt = del_blobs(cnt_str, cnt_nm, local_path+"/")
    except Exception as e:
        print(traceback.format_exc())
    return fnl_upload_status, fnl_upload_msg, blob_fld_name    

def upload_files_to_azure(min_no, max_no, accepted_file_type, file_size_accepted, cnt_str, cnt_nm, local_path, old_folder_name):
    global fld_name
    blob_fld_name = ''
    try:
        ...
        if no_of_files_status == 1:
            ...
            if check_filetype_status == 1:
                ...
                if fl_status == 1:
                    # Upload blobs:
                    del_blobs_status, del_blobs_msg, del_cnt = del_blobs(cnt_str, cnt_nm, old_folder_name)
                    upload_blobs_status, upload_blobs_msg, upld_cnt, blob_fld_name = upload_blobs(cnt_str, cnt_nm, local_path, old_folder_name)
                    upload_confirm_status, upload_confirm_msg = upload_confirm(cnt_str, cnt_nm, old_folder_name)
                    fnl_upload_status = 1
                    fnl_upload_msg = 'Files upload successful'
