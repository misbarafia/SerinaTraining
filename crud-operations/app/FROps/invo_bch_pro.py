...
import sys
sys.path.append("..")
from session.session import DB,USR,HOST,PWD,PORT

def invo_process(po_doc_id, invo_doc_id, documentLineitem_po_df, doc_header_po_df, documentLineitem_invo_df,
                 doc_header_invo_df, documentLineitem_grn_df, doc_inline_tags, ck_threshold, qty_threshold):
    doc_status = 0
    po_match_status = 1
    grn_match_status = 1
    # po_doc_id = 2737868
    ...
    req_po_inline = po_inlinedf[['idDocumentLineItems', 'lineItemtagID', 'Value', 'itemCode']].merge(doc_inline_tags,
                                                                                                     on='lineItemtagID',
                                                                                                     how='left')
    ...
    po_desc_df = pd.DataFrame(list(zip(po_desc, po_item_code)), columns=['ITEM_DESCRIPTION', 'itemCode'])
    ...
    po_qty_df = pd.DataFrame(list(zip(po_qty, po_qty_item_code)), columns=['QUANTITY', 'itemCode'])
    ...
    po_Uty_price_df = pd.DataFrame(list(zip(po_Uty_price, po_Uty_price_item_code)), columns=['UNIT_PRICE', 'itemCode'])
    po_Uty_price_df['itemCode'] = po_Uty_price_df['itemCode'].astype('str')
    po_tab = po_desc_df.merge(po_qty_df, on='itemCode').merge(po_Uty_price_df, on='itemCode')
    ...
    desc_df = pd.DataFrame(list(zip(desc, item_code)), columns=['Description', 'itemCode'])
    ...
    qty_df = pd.DataFrame(list(zip(qty, qty_item_code)), columns=['Quantity', 'itemCode'])
    Uty_price = inv_grouped_df.get_group('UnitPrice')['Value']
    Uty_price_item_code = inv_grouped_df.get_group('UnitPrice')['itemCode']
    Uty_price_df = pd.DataFrame(list(zip(Uty_price, Uty_price_item_code)), columns=['UnitPrice', 'itemCode'])
    ...
    invo_tab = desc_df.merge(qty_df, on='itemCode').merge(Uty_price_df, on='itemCode').merge(Amt_df, on='itemCode')
    invo_tab['Quantity'] = invo_tab['Quantity'].astype('float')
    invo_tab['UnitPrice'] = invo_tab['UnitPrice'].astype('float')

    # GRN DATA

    PO_HEADER_ID = list(po_header_df[po_header_df['TagLabel'] == 'PO_HEADER_ID']['Value'])[0]

    grn_inlinedf = documentLineitem_grn_df  # [documentLineitem_grn_df['documentID'] == 2737793]
    req_grn_df = grn_inlinedf[['idDocumentLineItems', 'documentID', 'lineItemtagID', 'Value', 'itemCode']].merge(
        doc_inline_tags, on='lineItemtagID', how='left')
    temp_grndf = req_grn_df[req_grn_df['TagName'] == 'PO_HEADER_ID']
    req_grn_docID = temp_grndf[temp_grndf['Value'] == PO_HEADER_ID]['documentID'].unique()[0]
    ...
    grn_qty_df = pd.DataFrame(list(zip(grn_qty, grn_qty_item_code)), columns=['QUANTITY', 'itemCode'])
    grn_qty_accepted = grn_grouped_df.get_group('QUANTITY_ACCEPTED')['Value']
    grn_qty_accepted_item_code = grn_grouped_df.get_group('QUANTITY_ACCEPTED')['itemCode']
    ...
    grn_qty_cancelled_item_code = grn_grouped_df.get_group('QUANTITY_CANCELLED')['itemCode']
    ...
    grn_qty_received_item_code = grn_grouped_df.get_group('QUANTITY_RECEIVED')['itemCode']
    ...
    grn_tab = grn_po_unitprice_df.merge(grn_qty_df, on='itemCode').merge(grn_qty_accepted_df, on='itemCode').merge(
        grn_qty_cancelled_df, on='itemCode').merge(grn_qty_received_df, on='itemCode').merge(grn_po_line_id_df,
                                                                                             on='itemCode')
    ...
    for invo_itm_cd in ck_matrix:  # invo_itm_cd == invo itemCode
        ...
        temp_code_map = {}
        mx_scr_item = max(ck_matrix[invo_itm_cd], key=ck_matrix[invo_itm_cd].get)  # mx_scr_item == po item code
        ...
        if mx_scr >= ck_threshold:
            ...
            if (po_qty == invo_qty) or (abs(po_qty - invo_qty) <= qty_threshold):
                ...
                status_scr = 0
                po_match_status_msg = 'PO Quantity mismatched with Invo Quantity'
                temp_code_map[mx_scr_item] = {'invo_itm_code': invo_itm_cd, 'fuzz_scr': mx_scr, 'status': status_scr,
                                              'po_qty': po_qty,
                                              'invo_qty': invo_qty, 'qty_mismatch': (po_qty - invo_qty),
                                              'po_match_status_msg': po_match_status_msg}
                # missing_items[mx_scr_item] = temp_code_map
                missing_items.update(temp_code_map)

            temp_code_map[mx_scr_item] = {'invo_itm_code': invo_itm_cd, 'fuzz_scr': mx_scr, 'invo_qty': invo_qty,'po_qty':po_qty,
                                          'status': status_scr,'qty_mismatch': (po_qty - invo_qty),'po_match_status_msg':po_match_status_msg}
            ...
            status_scr = 0
            po_match_status = po_match_status * 0
            po_match_status_msg = 'Invo item not matching with PO item with defined threshold'
            temp_code_map[mx_scr_item] = {'invo_itm_code': invo_itm_cd, 'fuzz_scr': mx_scr, 'status': status_scr,'po_qty':po_qty,
                                          'invo_qty': invo_qty, 'qty_mismatch': (invo_qty),'po_match_status_msg':po_match_status_msg}
            # unknown_items[mx_scr_item] = temp_code_map
            unknown_items.update(temp_code_map)
            print(ck_matrix[invo_itm_cd])

        item_code_map[mx_scr_item] = {'invo_itm_code': invo_itm_cd, 'fuzz_scr': mx_scr, 'status': status_scr,
                                      'invo_qty': invo_qty}

    missing_items_list = list(set(list(item_code_map.keys())) - set(list(po_tab['itemCode'])))
    ...
    for grn_itm in range(len(grn_qty_received_df['itemCode'])):
        ...
        grn_qty_status[grn_itm_code] = {'grn_status': grn_status, 'grn_status_msg': grn_status_msg, 'PO_qty': po_qty,
                                        'invoice_qty': invo_qty_ck, 'grn_qty': grn_qty}
    ...
    final_invo_pro_data = {"item_code_map": item_code_map, "grn_qty_status": grn_qty_status,
                           "missing_items": missing_items, "unknown_items": unknown_items, "doc_status": doc_status,"doc_status_msg":doc_status_msg}
...
def invo_bch_pro(SQL_USER, SQL_PASS, localhost, SQL_DB, SQL_PORT, ck_threshold, qty_threshold):
    ...
    doc_df = document_df[['idDocumentType', 'idDocument', 'documentStatusID', 'PODocumentID']]

    doc_ipType_df = docType_df.merge(doc_df, on='idDocumentType', how='left')

    doc_ipType_df.columns = ['idDocumentType', 'Name', 'documentID', 'documentStatusID', 'PODocumentID']

    doc_header_data_df = doc_header_data.merge(doc_ipType_df, on='documentID', how='left')

    documentLineitems_df = pd.read_sql_table("documentLineitems", SQLALCHEMY_DATABASE_URL)
    time.sleep(1)
    documentLineitemtags_df = pd.read_sql_table("documentLineitemtags", SQLALCHEMY_DATABASE_URL)
    time.sleep(1)
    ...
    doc_inline_data = doc_inline_data.merge(doc_inline_tags, on='lineItemtagID', how='left')
    documentLineitem_df = documentLineitems_df.merge(doc_ipType_df, on='documentID', how='left')
    ...
    documentLineitem_po_df = documentLineitem_df[documentLineitem_df['idDocumentType'] == 1]
    ...
    documentLineitem_invo_df = documentLineitem_df[documentLineitem_df['idDocumentType'] == 3]
    documentLineitem_df = documentLineitem_df.reset_index(drop=True)
    documentLineitem_grn_df = documentLineitem_df[documentLineitem_df['idDocumentType'] == 2]
    ...
    req_doc_header_invo_df = status_doc_invodf[status_doc_invodf['PODocumentID'] != 0].reset_index(drop=True)
    tmp_req_doc_header_invo_df = req_doc_header_invo_df[req_doc_header_invo_df['PODocumentID'] > 2737861]
    tmp_req_doc_header_invo_df = tmp_req_doc_header_invo_df[
        tmp_req_doc_header_invo_df['TagLabel'] == 'InvoiceId'].reset_index(drop=True)
    ...
    for inv_id in range(len(tmp_req_doc_header_invo_df)):
        ...
        final_invo_pro_data = invo_process(po_doc_id, invo_doc_id, documentLineitem_po_df, doc_header_po_df,
