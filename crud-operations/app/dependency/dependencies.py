...
async def check_create_user(u_id: int, db: scoped_session = Depends(get_db)):
    ...
    try:
        ...
        main_query = db.query(models.AccessPermissionDef.User).filter_by(idAccessPermissionDef=sub_query).scalar()
        if main_query == 1:
            return u_id
...
async def check_update_vendor_user(vu_id: int, db: scoped_session = Depends(get_db)):
    ...
    """
    try:
        sub_query = db.query(models.AccessPermission.permissionDefID).filter_by(userID=vu_id)
        main_query = db.query(models.AccessPermissionDef.Permissions).filter_by(
            idAccessPermissionDef=sub_query).scalar()
        if main_query == 1:
            return vu_id
        raise HTTPException(status_code=403, detail="Permission Denied")
    except Exception as e:
...
async def check_user_invoice(u_id: int, db: scoped_session = Depends(get_db)):
    ...
    try:
        sub_query = db.query(models.AccessPermission.permissionDefID).filter_by(userID=u_id)
        main_query = db.query(models.AccessPermissionDef.NewInvoice).filter_by(idAccessPermissionDef=sub_query).scalar()
        if main_query == 1:
            return u_id
        raise HTTPException(status_code=403, detail="Permission Denied")
    except Exception as e:
...
async def check_upload_vendor_user(vu_id: int, db: scoped_session = Depends(get_db)):
    ...
    """
    try:
        sub_query = db.query(models.AccessPermission.permissionDefID).filter_by(vendorUserID=vu_id)
        main_query = db.query(models.AccessPermissionDef.NewInvoice).filter_by(idAccessPermissionDef=sub_query).scalar()
        if main_query == 1:
            return vu_id
        raise HTTPException(status_code=403, detail="Permission Denied")
...
async def check_invoice_entity_user(u_id, inv_id, db: scoped_session = Depends(get_db)):
    ...
    try:
        ...
        # get iduseraccess to check if any access is avilable for the user on the given entity
        data = db.query(models.UserAccess.idUserAccess).filter_by(EntityID=sub_query.scalar_subquery(),
                                                                  UserID=u_id).all()
        if len(data) > 0:
            return True
        else:
...
def check_vendor_user_update(vu_id):
    ...
    try:
        ...
        permissionbool = db.query(models.AccessPermissionDef.Permissions).filter_by(
            idAccessPermissionDef=sub_query).one()
...
def check_eidt_invoice_approve_permission(u_id, inv_id):
    try:
        ...
        inv_perm = db.query(models.AccessPermissionDef.AccessPermissionTypeId).filter_by(
            idAccessPermissionDef=sub_query.scalar_subquery()).one()
...
def check_usertype(u_id, db: scoped_session = Depends(get_db)):
    try:
        ...
        user_type = db.query(models.Credentials.crentialTypeId).filter_by(userID=u_id).filter(
...
async def check_finance_approve(u_id, inv_id, db: scoped_session = Depends(get_db)):
    try:
        fn_perm = db.query(models.AccessPermission.permissionDefID).filter_by(
            userID=u_id).scalar()
        fn_perm = db.query(models.AccessPermissionDef.AccessPermissionTypeId).filter_by(
            idAccessPermissionDef=fn_perm).scalar()
        # check if document is of invoice type
        inv_type = db.query(models.Document.idDocument).filter_by(idDocument=inv_id, idDocumentType=3).scalar()
        if not inv_type:
            raise HTTPException(status_code=403, detail="Wrong invoice type")
        # check entity access
        val = await check_invoice_entity_user(u_id, inv_id, db)
        if not val:
            raise HTTPException(status_code=403, detail="Permission Denied, Entity Access not Available")
        # check if finance permission is there
        if fn_perm < 4:
            raise HTTPException(status_code=403, detail="Permission Denied, Request Admin for Upgrade")
    except Exception as e:
        print(e)
        raise HTTPException(status_code=403, detail="Permission Denied")
...
def check_user_access_customer(u_id, db: scoped_session = Depends(get_db)):
    try:
        # query to get user type
        user_type = db.query(models.Credentials.crentialTypeId).filter_by(userID=u_id).filter_by(
            crentialTypeId=1).scalar()
        if user_type:
            return
...
def check_if_user_amount_approval(maxamount: permissionssm.Maxamount, db: scoped_session = Depends(get_db)):
    try:
        # query to get user type
        pdef_id = db.query(models.AccessPermission.permissionDefID).filter_by(userID=maxamount.applied_uid).scalar()
        if pdef_id in (3, 9):
            return
...
def check_if_cust_user_and_admin(u_id, db: scoped_session = Depends(get_db)):
    try:
        # query to get user type
        type = db.query(models.Credentials.crentialTypeId).filter_by(userID=u_id, crentialTypeId=1).scalar()
        # query to get user permission
        def_id = db.query(models.AccessPermission.permissionDefID).filter_by(userID=u_id).scalar()
...
def check_if_service_trigger(u_id, db: scoped_session = Depends(get_db)):
    ...
    try:
        # query to get user permission
        def_id = db.query(models.AccessPermission.permissionDefID).filter_by(userID=u_id).scalar()
        allow_service_trig = db.query(models.AccessPermissionDef.allowServiceTrigger).filter_by(
            idAccessPermissionDef=def_id).scalar()
